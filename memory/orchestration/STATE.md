...
| GitHub Pages preview deploy | 2026-05-20T23:58:00Z | Pass | Pushed main through UI commit 6ec977d; GitHub Actions run 26196866000 completed build and deploy successfully. Public preview fetch verified the redesigned home page, Roger Marshall dossier/source trail, elections index, State Treasurer race, and KS House District 110 race with no targeted forbidden/internal wording. |
| Cloudflare Pages deploy | 2026-05-20T20:38:00-05:00 | Partial | Created Cloudflare Pages project `elect-righteous`, built root-domain export with `NEXT_PUBLIC_SITE_URL=https://electrighteous.com`, deployed via Wrangler, and verified `https://elect-righteous.pages.dev/` serves the current root-domain build. |
| Production domain verification | 2026-05-20T20:38:00-05:00 | Fail/blocked | Pages custom domains `electrighteous.com` and `www.electrighteous.com` were added, but both remain pending because DNS records are missing. Current Wrangler OAuth token cannot create DNS records: Cloudflare DNS API returns 403 authentication errors. |
